...
#enable job control
set -m
vpkg_require cuda/12.4.1-550.54.15
...
nvidia-smi --query-gpu="${QUERY_LIST[*]}" --format=csv,nounits \
    --loop-ms=${MS_INTERVAL} --filename=nv_smi_out.csv &

#busy wait until nvidia-smi opens log file in r/w mode
until fuser -s nv_smi_out.csv; do
 :
done

#process to execute here
sleep 1
